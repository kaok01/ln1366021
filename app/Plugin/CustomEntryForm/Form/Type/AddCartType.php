...
class AddCartType extends AbstractType
{
        ...
        if ($Product->getStockFind()) {
            ...
            if ($Product && $Product->getProductClasses()) {
                if (!is_null($Product->getClassName1())) {
                    $data = $Product->getClassCategories1();
                    $builder->add('classcategory_id1', 'choice', array(
                        'label' => 'sssss'.$Product->getClassName1(),
                ...
                if (!is_null($Product->getClassName2())) {
                    $data= $Product->getClassCategories2(array_keys($data)[0]);
                    $builder->add('classcategory_id2', 'choice', array(
                        'label' => 'fffff'.$Product->getClassName2(),
                        'choices' => $data,
                        'expanded'   => true,
                        'data' => array_keys($data)[0],
                    ));
                ...
                $builder->add('product_class_id', 'choice', array(
                    'label' => 'sssss'.$Product->getClassName1(),
                    'choices' => $pcs,
                    'data' => array_keys($pcs)[0],
                    'expanded'   => true,

                ));

                $pcr=array();
                $pcolors = $this->app['eccube.plugin.productcolor.repository.product_productcolor']->findBy(array('Product'=>$Product));
                foreach($pcolors as $pc){
                    $pcr[$pc->getProductColor()->getId()]=$pc->getProductColor();

                }
                $builder->add('product_color', 'choice', array(
                    'label' => '商品カラー',
                    'choices' => array(0=>'相談して決める')+$pcr,
                    'data' => array_keys($pcr)[0],
                    'expanded'   => true,
            ...
            $builder->addEventListener(FormEvents::PRE_SUBMIT, function (FormEvent $event) use ($Product) {
                ...
                $form = $event->getForm();
                if (!is_null($Product->getClassName2())) {
                    $data= $Product->getClassCategories2($data['classcategory_id1']);
                    if ($data) {
    ...
    {
        return 'customentryform_add_cart';
    }
